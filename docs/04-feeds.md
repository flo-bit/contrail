...
Doing one slice per tick keeps each tick's cost flat no matter how big the table grows. The recovery timer measures from the last *completed full pass* (not the last slice), so when a pass is overdue the sweep keeps taking a slice every tick until the cursor wraps. That guarantees the whole table is cleaned at least once per recovery interval, instead of just one slice every interval.
