...
`contrail lexicons all` also pulls referenced record Lexicons and runs Atcute TypeScript generation. Use `--public` when generating only methods advertised by the anonymous read surface. The generator is also exported from `@atmo-dev/contrail/lexicons` for programmatic use.
